...
- 前后端只允许访问 `/scan`。后端会解析符号链接并拒绝逃逸路径；`IS_TIMEDOCK=Y` 时还会按登录用户的 `timedock_account` 追加目录限制。
- 业务资源按用户名隔离。admin 不是“全局数据查看者”，只额外拥有用户管理、服务配置和 ClamAV 全局控制权限。
...
历史记录、统计和隔离区查询采用“创建 lookup → 轮询结果”的模式，避免大目录或大量任务阻塞 HTTP 请求。lookup 存在内存中，重启即失效；前端调用时应接受 `202` 并轮询对应资源，具体请求形状见 API 文档。
...
- 保持 API 的用户隔离：新资源必须绑定当前 `actor.Username`，跨用户访问最好返回 `404`，不要泄露资源存在性。
